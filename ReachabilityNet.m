...
//  ShipleySchool
//
//  Created by Sudip Deshpande on 9/7/11.
...
static NSString *kLinkLocalAddressKey = @"192.168.0.107";
static NSString *kDefaultRouteKey = @"0.0.0.0";

static ReachabilityNet *_sharedReachability;
// A class extension that declares internal methods for this class.
@interface ReachabilityNet()
...
@implementation ReachabilityNet
...
+ (ReachabilityNet *)sharedReachability
{
	if (!_sharedReachability) {
		_sharedReachability = [[ReachabilityNet alloc] init];
...
- (void)scheduleOnRunLoop:(NSRunLoop *)inRunLoop
{
	...
	if ([[ReachabilityNet sharedReachability] networkStatusNotificationsEnabled] == NO) {
		return;
	}
